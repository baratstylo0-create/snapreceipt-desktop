...
test('Electron package includes every runtime module imported by main', () => {
  ...
  assert.ok(packageJson.build.files.includes('config.js'));
  assert.ok(packageJson.build.files.includes('google-auth.js'));
});

test('Store package preparation refuses missing Partner Center identity metadata', () => {
  const result = spawnSync(process.execPath, [script], { cwd: repoRoot, env: { ...process.env }, encoding: 'utf8' });
...
test('Store package preparation writes only the supplied identity metadata', () => {
  const previousConfig = fs.existsSync(generatedConfig) ? fs.readFileSync(generatedConfig) : null;
  try {
