...
const values = Object.fromEntries(required.map((name) => [name, String(process.env[name] || '').trim()]));
const missing = required.filter((name) => !values[name]);
if (missing.length > 0) {
  throw new Error('Refusing to create a Microsoft Store package without Partner Center metadata: ' + missing.join(', '));
}
...
if (values.WINDOWS_STORE_APPLICATION_ID.length < 1 || values.WINDOWS_STORE_APPLICATION_ID.length > 64 ||
  ...
  throw new Error('WINDOWS_STORE_APPLICATION_ID must use valid AppX application-id segments.');
}
...
fs.writeFileSync(path.join(__dirname, '..', 'store-build-config.json'), `${JSON.stringify(config, null, 2)}\n`, { mode: 0o600 });
