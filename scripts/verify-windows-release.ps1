...
# Some developer shells prepend alternate PowerShell module trees. Restrict this
# verification process to the Windows PowerShell modules so Authenticode validation
# cannot load duplicate type metadata from a second provider.
...
if (-not $signature.SignerCertificate) {
  throw 'Release artifact has no signer certificate.'
}
